...
let SellersService = SellersService_1 = class SellersService {
    ...
    async updateBranding(sellerId, raw) {
        ...
        if (data.primaryColor && typeof data.primaryColor === 'string') {
            if (!data.primaryColor.startsWith('#'))
                data.primaryColor = `#${data.primaryColor}`;
            if (!HEX.test(data.primaryColor))
                throw new common_1.BadRequestException('Color primario inválido');
        }
    ...
    async create(data) {
        ...
        if (!name || !email || !password || !slug) {
            throw new common_1.BadRequestException('Faltan datos obligatorios (name, email, password, slug)');
        }
        const [existing] = await this.db
            .select({ id: db_1.sellers.id })
            .from(db_1.sellers)
            .where((0, drizzle_orm_1.eq)(db_1.sellers.slug, slug.toLowerCase()))
            .limit(1);
        if (existing)
            throw new common_1.BadRequestException('Ya existe un comercio con ese slug');
        ...
        await this.db.insert(db_1.sellerBranding).values({ sellerId: created.id }).onConflictDoNothing();
        return created;
    }
};
